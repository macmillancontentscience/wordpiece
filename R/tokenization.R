...
load_vocab <- function(vocab_file) {
    ...
    attr(index_list, "is_cased") <- is_cased
    return(index_list)
}
...
.strip_accents <- function(text) {
    ...
    return(
        .apply_to_chars(text,
                        function(char) {
                            if(stringi::stri_detect_charclass(char,
                                                              "\\p{Mn}")) {
                                return("")
                            }
                            return(char)
...
.is_whitespace <- function(char) {
    ...
    # Maybe enforce that char is *single* character here?
...
#' @param vocab The word piece vocabulary to use.
#'
#' @return An named integer vector, giving the tokenization of the input
...
#' )
#' }
tokenize_single_sequence <- function(text,
                                     vocab,
                                     unk_token = "[UNK]",
                                     max_chars = 100) {
    ...
    text <- purrr::map(whitespace_tokenize(text),
                       # tokenize_word is the part we'll modify in morphemizer.
                       .f = tokenize_word,
                       vocab = vocab,
                       unk_token = unk_token,
                       max_chars = max_chars)
...
.infer_case_from_vocab <- function(vocab) {
    ...
    is_cased <- any(stringi::stri_detect(str = names(vocab), regex = "^[A-Z]"))
    return(is_cased)
}
